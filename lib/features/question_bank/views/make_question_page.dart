...
import '../../../core/widgets/common_state_widget.dart'; // ✅ 导入通用状态组件
...
class MakeQuestionPage extends ConsumerStatefulWidget {
  ...
  const MakeQuestionPage({this.extra, super.key});
...
class _MakeQuestionPageState extends ConsumerState<MakeQuestionPage> {
  ...
  String? _errorMessage; // 错误信息（网络请求失败时显示）
    ...
    if (mode == 2) {
      ...
      if (_savedAnswerStates.isNotEmpty) {
        for (var question in _questions) {
          final questionId = question['id']?.toString();
          if (questionId != null &&
              _savedAnswerStates.containsKey(questionId)) {
  ...
  Future<void> _loadQuestions() async {
    ...
    try {
      ...
      setState(() {
        _isLoading = false;
        _errorMessage = e.toString(); // 记录错误信息，不显示假数据
      });
    }
  }
  ...
  List<Map<String, dynamic>> _transformQuestions(
    ...
    for (int i = 0; i < questionList.length; i++) {
      ...
      if (stemList.length == 1) {
        ...
        print(
          '✅ [拆分配伍题] 第${i + 1}题 配伍题，stem_list=${stemList.length}，type=$questionType，是否B1=$isB1，组号=$groupCounter',
        );
        ...
        for (int j = 0; j < stemList.length; j++) {
          ...
          print(
            '   - 拆分子题 ${j + 1}/${stemList.length}: stem_id=${singleStem['id']}${isB1 ? "，B1第${groupCounter}组第${j + 1}题" : ""}',
          );
        }
      }
    }
  ...
  Map<String, dynamic> _processQuestionData(Map<String, dynamic> rawQuestion) {
        ...
        if (matchedOption != null) {
          ...
          if (answerList != null && answerList.isNotEmpty) {
            ...
            userAnswer = answerList
                ...
                .join('');
            print(
              '✅ [已做题目] stem_id=$stemId, user_option答案=$answerList, 转换为=$userAnswer',
            );
          }
        }
      } catch (e) {
    ...
    final processed = {
      'id':
          rawQuestion['id']?.toString() ??
          rawQuestion['question_id']?.toString() ??
          '',
      'question_id':
          rawQuestion['question_id']?.toString() ??
          rawQuestion['id']?.toString() ??
      ...
      'original_stem_list':
          rawQuestion['original_stem_list'], // ⚠️ 关键：保存原始完整的小题列表（提交时需要）
      'current_stem_index':
          rawQuestion['current_stem_index'] ?? 0, // ⚠️ 关键：保存当前小题索引
    ...
    print(
      '   - 题干: ${questionText.substring(0, questionText.length > 30 ? 30 : questionText.length)}...',
    );
    ...
    print(
      '   - stem_list长度: ${(rawQuestion['stem_list'] as List?)?.length ?? 0}',
    );
    print(
      '   - original_stem_list长度: ${(rawQuestion['original_stem_list'] as List?)?.length ?? 0}',
    );
    print('   - current_stem_index: ${rawQuestion['current_stem_index'] ?? 0}');
    print(
      '   - B1配伍题: ${processed['is_b1_group']}${processed['is_b1_group'] == true ? '（第${processed['b1_group_number']}组第${processed['b1_sub_index']}题）' : ''}',
    );
  ...
  Widget build(BuildContext context) {
    ...
    if (_errorMessage != null) {
      return Scaffold(
        ...
        body: CommonStateWidget.networkError(
          onRetry: () {
            setState(() {
              _isLoading = true;
              _errorMessage = null;
              ...
              Expanded(
                child: QuestionCardSwiper(
                  ...
                  onAnswerChanged: (data) {
                    ...
                    print(
                      '👆 [选项点击] 题目${_questions[index]['id']}, 答案: $answer',
                    );
                  },
                ),
              ),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          if (_currentMode == 1 &&
              _allQuestions.any((q) => q['is_correct'] == false))
            GestureDetector(
              onTap: _toggleErrorOnly,
              child: Container(
                padding: EdgeInsets.symmetric(horizontal: 12.w, vertical: 6.h),
                decoration: BoxDecoration(
                  color: _showOnlyError
                      ? AppColors.tikuPrice
          ...
          if (_recitationQuestionModel == 1) _buildModeSwitch(),
  ...
  Widget _buildModeSwitch() {
    return Container(
      ...
      child: Row(
        ...
        children: [_buildModeButton(1, '答题'), _buildModeButton(2, '背题')],
      ),
    );
  }
...
''';
      ...
      // ✅ 调用系统分享
      await Share.share(shareText, subject: '金医圣题库 - 问老师');
  ...
  Future<void> _submitAnswers() async {
    ...
    try {
      ...
      final answeredQuestions = _allQuestions.where((q) {
        return q['user_answer'] != null &&
            q['user_answer'].toString().isNotEmpty;
      }).toList();
      ...
      final Map<String, List<int>> answeredStemIndexes =
          {}; // 记录每个question_id已答的小题索引
      ...
      print(
        '📊 [分组去重] 原始题目数: ${answeredQuestions.length}, 去重后: ${questionMap.length}',
      );
      ...
      final List<Map<String, dynamic>> groupedQuestions = questionMap.values
          .toList();
      ...
      final questionInfo = groupedQuestions.map((q) {
        ...
        print(
          '📝 [构建提交数据] question_id=$questionId, 原始小题数=${originalStemList.length}, 当前小题索引=$currentStemIndex',
        );
          ...
          if (answeredIndexes.contains(index)) {
            ...
            final answeredQuestion = answeredQuestions.firstWhere(
              (aq) =>
                  ...
                  aq['current_stem_index'] == index,
              orElse: () => <String, dynamic>{},
            );

            if (answeredQuestion.isNotEmpty &&
                answeredQuestion['user_answer'] != null) {
            ...
            print(
              '   - 小题$index (已答): sub_question_id=$subQuestionId, answer=$answerList',
            );
          } else {
            // ✅ 该小题未答，answer为空数组
            answerList = [];
            print(
              '   - 小题$index (未答): sub_question_id=$subQuestionId, answer=[]',
            );
          }
      ...
      print(
        '   - teaching_system_package_id: $_teachingSystemPackageId',
      ); // ⚠️ 添加日志
