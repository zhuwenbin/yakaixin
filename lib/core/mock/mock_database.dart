...
class MockDatabase {
  ...
  static List<Map<String, dynamic>>? _goodsDetailTable;
  ...
  static Future<void> _performInit() async {
    ...
    try {
      // 加载商品数据
      final goodsJson = await rootBundle.loadString(
        'assets/mock/goods_data.json',
      );
      ...
      // 加载订单数据
      final orderJson = await rootBundle.loadString(
        'assets/mock/order_data.json',
      );
      ...
      _chapterTable = (chapterData['data'] as List)
          .cast<Map<String, dynamic>>();
      print('   ✅ 章节数据加载成功: ${_chapterTable!.length} 条');

      // 加载每日一测数据
      final dailyJson = await rootBundle.loadString(
        'assets/mock/daily_practice_data.json',
      );
      final dailyData = jsonDecode(dailyJson) as Map<String, dynamic>;
      _dailyPracticeTable = (dailyData['data'] as List)
          .cast<Map<String, dynamic>>();
      print('   ✅ 每日一测数据加载成功: ${_dailyPracticeTable!.length} 条');

      // 加载技能模拟数据
      final skillJson = await rootBundle.loadString(
        'assets/mock/skill_mock_data.json',
      );
      final skillData = jsonDecode(skillJson) as Map<String, dynamic>;
      _skillMockTable = (skillData['data'] as List)
          .cast<Map<String, dynamic>>();
      print('   ✅ 技能模拟数据加载成功: ${_skillMockTable!.length} 条');

      // 加载学习中心数据（calendar/lessons/plan）
      final studyJson = await rootBundle.loadString(
        'assets/mock/study_data.json',
      );
      ...
      // 加载商品详情数据
      final goodsDetailJson = await rootBundle.loadString(
        'assets/mock/goods_detail_data.json',
      );
      final goodsDetailData =
          jsonDecode(goodsDetailJson) as Map<String, dynamic>;
      _goodsDetailTable = (goodsDetailData['data'] as List)
          .cast<Map<String, dynamic>>();
      print('   ✅ 商品详情数据加载成功: ${_goodsDetailTable!.length} 条');
  ...
  /// - page: 页码
  /// - page_size: 每页数量
  static Future<Map<String, dynamic>> queryGoods(
  ...
  /// 参数:
  /// - code: 配置代码 (如 PUBLISH)
  static Future<Map<String, dynamic>> queryConfig(
    Map<String, dynamic> params,
  ) async {
    // 确保已初始化
    await init();
    ...
    if (code == null || !_configTable!.containsKey(code)) {
      print('   ❌ 配置不存在');
      return {'code': 0, 'msg': '配置不存在', 'data': null};
    }
    ...
    return {'code': 200, 'msg': 'success', 'data': value};
  }
  ...
  static Future<Map<String, dynamic>> queryLearningData(
    Map<String, dynamic> params,
  ) async {
    await init();
    ...
    return {'code': 100000, 'msg': 'success', 'data': filteredData[0]};
  }
  ...
  /// 参数:
  /// - professional_id: 专业ID
  static Future<Map<String, dynamic>> queryChapters(
    Map<String, dynamic> params,
  ) async {
    await init();
    ...
    return {'code': 100000, 'msg': 'success', 'data': filteredData};
  }
  ...
  static Future<Map<String, dynamic>> queryCourseChapters(
    Map<String, dynamic> params,
  ) async {
    await init();
    ...
    return {'code': 100000, 'msg': '操作成功', 'data': mockChapters};
  }
  ...
  static Future<Map<String, dynamic>> queryDailyPractice(
    Map<String, dynamic> params,
  ) async {
    await init();
    ...
    final filteredData = _dailyPracticeTable!.where((item) {
      final matchProfessional =
          professionalId == null || item['professional_id'] == professionalId;
      final matchPosition =
          positionIdentify == null ||
          item['position_identify'] == positionIdentify;
  ...
  static Future<Map<String, dynamic>> querySkillMock(
    Map<String, dynamic> params,
  ) async {
    await init();
  ...
  static Future<Map<String, dynamic>> checkin(
    ...
    if (index != -1) {
      ...
      _learningDataTable![index]['checkin_num'] =
          (_learningDataTable![index]['checkin_num'] as int) + 1;
    }

    return {'code': 100000, 'msg': '打卡成功', 'data': null};
  }
  ...
  static Future<Map<String, dynamic>> queryStudyCalendar(
    Map<String, dynamic> params,
  ) async {
    await init();
    ...
    return {'code': 100000, 'msg': 'success', 'data': calendarData};
  }
  ...
  /// 参数:
  /// - date: 日期 (yyyy-MM-dd)
  static Future<Map<String, dynamic>> queryStudyLessons(
    Map<String, dynamic> params,
  ) async {
    await init();
    ...
    final lessonResponse =
        _studyDataTable!['lesson_response'] as Map<String, dynamic>;

    // 这里可以根据日期筛选，暂时返回所有数据
    return {'code': 100000, 'msg': 'success', 'data': lessonResponse};
  }
  ...
  /// - date: 日期 (yyyy-MM-dd)
  static Future<Map<String, dynamic>> queryStudyPlan(
    Map<String, dynamic> params,
  ) async {
    await init();
    ...
    return {'code': 100000, 'msg': 'success', 'data': filteredCourses};
  }

  /// 查询商品详情
  ///
  /// 对应接口: GET /c/goods/v2/detail
  /// 参数:
  /// - goods_id: 商品ID (必需)
  static Future<Map<String, dynamic>> queryGoodsDetail(
    Map<String, dynamic> params,
  ) async {
    await init();

    final goodsId = params['goods_id'] as String?;
    print('🗄️ Mock数据库查询 - 商品详情');
    print('   商品ID: $goodsId');
    ...
    print('   ✅ 查询成功: ${detail['name']}');

    // ✅ 为课程商品添加 detail_package_goods（对应小程序 newVideo.vue Line 400）
    // 这是套餐包含的子商品列表，用于获取章节目录
    final result = Map<String, dynamic>.from(detail);
    if (!result.containsKey('detail_package_goods')) {
      result['detail_package_goods'] = [
        {
          'id': '1', // ✅ 这个ID会被用于查询章节
          'name': '基础阶段课程',
        },
